...
export type SocketClientToServerEvents = {
  ...
  'draw:start': (data: { x: number; y: number; color: string; width: number; isEraser?: boolean }) => void;
  'draw:move': (data: { x: number; y: number }) => void;
  'draw:end': () => void;
  'draw:clear': () => void;
}
...
export type SocketServerToClientEvents = {
  ...
  'draw:start': (payload: { userId: string; points: Point[]; color: string; strokeWidth: number; isEraser?: boolean }) => void;
  'draw:move': (payload: { userId: string; points: Point[] }) => void;
  'draw:end': (payload: { userId: string }) => void;
  'draw:clear': () => void;
}

export type GetEndpoints = {
  'users': {users: User[]};
  'strokes': {strokes: { userId: string; points: Point[]; color: string; strokeWidth: number; isEraser?: boolean; isComplete: boolean; timestamp: number }[]}
}
...
class _SocketManager {
  ...
  listen<K extends keyof SocketServerToClientEvents>(
    ...
    console.log('LISTEN', { eventName, socketManger: this.socketManager });
