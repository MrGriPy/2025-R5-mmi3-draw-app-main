...
import { addDrawStroke, updateDrawStroke, completeDrawStroke, clearAllStrokes, getAllStrokes, startAutomaticCleanup } from './draw';
...
export const handleAppForClient = (app: ReturnType<typeof express>, io: Server, socket: Socket) => {
  ...
  socket.on(CLIENT_TO_SERVER_EVENTS_NAMES.DRAW_START, (data: { x: number; y: number; color: string; width: number; isEraser?: boolean }) => {
    logListen(CLIENT_TO_SERVER_EVENTS_NAMES.DRAW_START, data);
    
    const { x, y, color, width, isEraser } = data;
    const userId = socket.id;
    
    const stroke = addDrawStroke(userId, { x, y }, color, width, isEraser);
    editUser(userId, { hasDrawn: true });
    
    emitToAll(SERVER_TO_CLIENT_EVENTS_NAMES.USERS_UPDATED, { users: getUsers() });

    
    emitToAllButSender(SERVER_TO_CLIENT_EVENTS_NAMES.DRAW_START, {
  ...
  socket.on(CLIENT_TO_SERVER_EVENTS_NAMES.DRAW_END, () => {
    ...
    if (stroke) {
      emitToAllButSender(SERVER_TO_CLIENT_EVENTS_NAMES.DRAW_END, {
        userId: stroke.userId
      });
    }
  });
